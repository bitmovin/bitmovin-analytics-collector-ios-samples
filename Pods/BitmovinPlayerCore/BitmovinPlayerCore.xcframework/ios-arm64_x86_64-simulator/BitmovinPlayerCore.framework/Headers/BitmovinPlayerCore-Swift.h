...
@interface BMPAdItem (SWIFT_EXTENSION(BitmovinPlayerCore)) <NSCopying>
...
+ (void)setLogging:(BMPLoggingConfig * _Nonnull)value;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// This method will throw an error in case a <code>SourceConfig</code> is passed that is not supported for being downloaded.
/// A <code>SourceConfig</code> is only valid for download when it’s configured using an HLS asset.
/// If an <code>OfflineContentManager</code> is acquired using this method, subsequent calls to
/// any content related API on the <code>OfflineManager</code> are not supported, meaning
/// they could result in unexpected behaviour.
/// \param for A <code>SourceConfig</code> for which the <code>OfflineContentManger</code> is requested.
///
/// \param id A unique identifier for the given <code>SourceConfig</code> which must not change once provided.
///
///
/// returns:
...
///
/// \param for The identifier which is provided by the system.
///
...
- (id <_BMPFairplayHandler> _Nonnull)createWithFairplayConfig:(BMPFairplayConfig * _Nonnull)fairplayConfig networkConfig:(BMPNetworkConfig * _Nullable)networkConfig offlineLicenseStorage:(id <_BMPOfflineFairplayLicenseStorage> _Nullable)offlineLicenseStorage loggerService:(id <_BMPObjCLoggerService> _Nonnull)loggerService SWIFT_WARN_UNUSED_RESULT;
@end
...
@interface _BMPDefaultUrlAssetValuesLoader (SWIFT_EXTENSION(BitmovinPlayerCore))
...
@interface _BMPPersistentArchive (SWIFT_EXTENSION(BitmovinPlayerCore))
